import{s as n}from"./supabaseClient-B1HaFb4P.js";import{A as k}from"./AdminNavbar-DXVtneOk.js";import{r as E}from"./adminAuth-Dn35BI8v.js";import{s as u}from"./toast-DRvdR0y9.js";import{s as v}from"./sendEmail-89Z52C2k.js";import{s as $}from"./signupHandler-BTzv1DSv.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm";import"./logo-nobg-BVMQOOtC.js";import"./logo-yCyWWFG1.js";import"./email-cNRN05tX.js";const f="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imlmb2RicXlnc2Nkc3h4bHhmanh3Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1ODQ3MzEyOCwiZXhwIjoyMDc0MDQ5MTI4fQ.hgTFx1O5bbgwJvTJlY4xXG3DS7eh1m1QRReLoefSD0E",I="https://ifodbqygscdsxxlxfjxw.supabase.co";async function S(l){return(await fetch(`${I}/auth/v1/admin/users/${l}`,{method:"DELETE",headers:{apikey:f,Authorization:`Bearer ${f}`,"Content-Type":"application/json"}})).ok}function x(){return`<div class="flex justify-center items-center py-16">
  ...
  `}const q=async()=>{if(!await E())return{html:"",pageEvents:()=>{}};let l="users",e=!1,d=localStorage.getItem("admin_dark")==="true",{data:_=[]}=await n.from("profiles").select("*").order("created_at",{ascending:!1}),{data:h=[]}=await n.from("accounts").select("*"),{data:w=[]}=await n.from("crypto_balances").select("*"),b=_.map(r=>{const p=h.find(s=>s.user_id===r.id)||{},o=w.find(s=>s.user_id===r.id)||{};return{...r,account_type:p.account_type||"-",account_id:p.id,is_active:p.is_active!==!1&&r.is_active!==!1,balance:parseFloat(p.balance||0).toFixed(2),btc_balance:parseFloat(o.btc_balance||0).toFixed(8),eth_balance:parseFloat(o.eth_balance||0).toFixed(8),usdt_balance:parseFloat(o.usdt_balance||0).toFixed(6),usdc_balance:parseFloat(o.usdc_balance||0).toFixed(6),bnb_balance:parseFloat(o.bnb_balance||0).toFixed(8),sol_balance:parseFloat(o.sol_balance||0).toFixed(8)}}),y=b;function i(){document.getElementById("app").innerHTML=`
  ...
  `,document.getElementById("admin-sidebar");const r=document.getElementById("admin-sidebar-overlay"),p=document.getElementById("admin-sidebar-toggle"),o=document.getElementById("admin-sidebar-close");p?.addEventListener("click",()=>{e=!1,i()}),o?.addEventListener("click",()=>{e=!0,i()}),r?.addEventListener("click",()=>{e=!0,i()}),document.getElementById("admin-theme-toggle")?.addEventListener("click",()=>{d=!d,localStorage.setItem("admin_dark",d?"true":"false"),document.documentElement.classList.toggle("dark",d),i()}),document.getElementById("admin-theme-toggle-desktop")?.addEventListener("click",()=>{d=!d,localStorage.setItem("admin_dark",d?"true":"false"),document.documentElement.classList.toggle("dark",d),i()}),document.getElementById("user-search")?.addEventListener("input",function(){const s=this.value.trim().toLowerCase();y=b.filter(a=>a.full_name&&a.full_name.toLowerCase().includes(s)||a.email&&a.email.toLowerCase().includes(s)),i()}),document.getElementById("add-user-btn")?.addEventListener("click",()=>{document.getElementById("user-modal-panel").innerHTML=g({mode:"create"}),document.getElementById("close-user-form").onclick=()=>{document.getElementById("user-modal-panel").innerHTML=""},document.getElementById("cancel-user-form").onclick=()=>{document.getElementById("user-modal-panel").innerHTML=""},document.getElementById("user-form").onsubmit=async function(s){s.preventDefault(),document.getElementById("user-modal-panel").innerHTML+=x();const a=Object.fromEntries(new FormData(this));try{await $(a,"admin"),u("User created!","success"),location.reload()}catch(c){u(c.message,"error"),document.querySelector(".flex.justify-center.items-center.py-16")?.remove()}}}),document.querySelectorAll(".user-edit").forEach(s=>{s.onclick=()=>{const a=b.find(c=>c.id===s.dataset.id);document.getElementById("user-modal-panel").innerHTML=g({mode:"edit",user:a}),document.getElementById("close-user-form").onclick=()=>{document.getElementById("user-modal-panel").innerHTML=""},document.getElementById("cancel-user-form").onclick=()=>{document.getElementById("user-modal-panel").innerHTML=""},document.getElementById("user-form").onsubmit=async function(c){c.preventDefault(),document.getElementById("user-modal-panel").innerHTML+=x();const t=Object.fromEntries(new FormData(this));try{await n.from("profiles").update({full_name:t.full_name,title:t.title,firstname:t.firstname,lastname:t.lastname,phone:t.phone,country_code:t.country_code,nationality:t.nationality,address:t.address,city:t.city,state:t.state,zip:t.zip,dob:t.dob,occupation:t.occupation,ssn:t.ssn,marital_status:t.marital_status,gender:t.gender}).eq("id",a.id),await n.from("accounts").update({account_type:t.account_type,is_active:t.is_active==="true",balance:parseFloat(t.balance)||0}).eq("user_id",a.id);const{data:m}=await n.from("crypto_balances").select("id").eq("user_id",a.id).eq("account_id",a.account_id).single();await n.from("crypto_balances").upsert([{id:m?.id,user_id:a.id,account_id:a.account_id,btc_balance:parseFloat(t.btc_balance)||0,eth_balance:parseFloat(t.eth_balance)||0,usdt_balance:parseFloat(t.usdt_balance)||0,usdc_balance:parseFloat(t.usdc_balance)||0,bnb_balance:parseFloat(t.bnb_balance)||0,sol_balance:parseFloat(t.sol_balance)||0}]),await v({to:a.email,subject:"Profile Updated",html:`<p>Hello <b>${t.full_name}</b>,<br>Your profile was updated by an admin.</p>`}),u("Profile updated!","success"),location.reload()}catch(m){u(m.message,"error"),document.querySelector(".flex.justify-center.items-center.py-16")?.remove()}}}}),document.querySelectorAll(".user-delete").forEach(s=>{s.onclick=async()=>{const a=b.find(c=>c.id===s.dataset.id);if(confirm(`Delete ${a.full_name}? This cannot be undone.`)){document.getElementById("user-modal-panel").innerHTML+=x();try{await n.from("login_otps").delete().eq("user_id",a.id),await n.from("crypto_balances").delete().eq("user_id",a.id),await n.from("accounts").delete().eq("user_id",a.id),await n.from("profiles").delete().eq("id",a.id),await S(a.id),await v({to:a.email,subject:"Profile Deleted",html:`<p>Hello <b>${a.full_name}</b>,<br>Your profile has been deleted by an admin.</p>`}),u("Profile deleted!","success"),location.reload()}catch(c){u("Failed to delete user: "+c.message,"error"),document.querySelector(".flex.justify-center.items-center.py-16")?.remove()}}}}),document.documentElement.classList.toggle("dark",d)}return{html:"",pageEvents:()=>i()}};export{q as default};
