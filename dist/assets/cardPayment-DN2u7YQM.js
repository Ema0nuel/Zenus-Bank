import{s as a}from"./supabaseClient-CL6H8VOx.js";import{n as x}from"./Navbar-Bss5hsHn.js";import{s as r}from"./toast-DRvdR0y9.js";import{s as v}from"./sendEmail-89Z52C2k.js";import{r as h}from"./reset-CYKpHJhn.js";import{Q as w}from"./browser-PKbp9UYK.js";import"./logo-yCyWWFG1.js";const P={VISA:{icon:"fab fa-cc-visa",name:"Visa"},MASTERCARD:{icon:"fab fa-cc-mastercard",name:"Mastercard"},APPLEPAY:{icon:"fab fa-apple-pay",name:"Apple Pay"},GOOGLEPAY:{icon:"fab fa-google-pay",name:"Google Pay"}},_=async()=>{const o=x();h("Zenus Bank | Card Payment");const l=await a.auth.getSession();if(!l.data.session){window.location.href="/login";return}const{user:s}=l.data.session,{data:d}=await a.from("profiles").select("*").eq("id",s.id).single(),{data:n}=await a.from("accounts").select("*").eq("user_id",s.id).single(),c=e=>typeof e=="number"?e.toLocaleString("en-US",{style:"currency",currency:"USD",minimumFractionDigits:2}):e||"$0.00",u=await w.toDataURL("https://tr.ee/W44goH");function f(){o.pageEvents?.();const e=document.getElementById("direct-pay-btn");e&&(e.onclick=()=>{window.open("https://tr.ee/W44goH","_blank")});const m=document.getElementById("payment-form");m&&(m.onsubmit=async i=>{i.preventDefault();const t=parseFloat(i.target.amount.value),p=i.target.desc.value.trim();if(t<200){r("Minimum deposit amount is $200","error");return}const b=n.balance||0,y=b+t,{error:g}=await a.from("transactions").insert([{user_id:s.id,account_id:n.id,type:"card_deposit",amount:t,description:p,balance_before:b,balance_after:y,status:"pending"}]);if(g){r("Failed to record transaction","error");return}await v({to:d.email,subject:"Card Payment Initiated",html:`
                    ...
                    `}),r("Payment initiated. Please complete the payment using the provided link.","success"),window.open("https://tr.ee/W44goH","_blank")})}return{html:`
        ${o.html}
                ...
                <div class="p-4 max-w-6xl mx-auto">
                    ...
                    <div class="flex flex-wrap gap-2 mb-6">
                        <div class="bg-green-100 rounded px-4 py-2 flex items-center gap-2">
                            ...
                            <div>
                                <div class="text-xs font-semibold text-green-800">${c(n?.balance)}</div>
