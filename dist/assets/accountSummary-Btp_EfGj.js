import{s as r}from"./supabaseClient-CvVHAHOf.js";import{n as f}from"./Navbar-ElK9Prgf.js";import{r as u}from"./reset-CYKpHJhn.js";import{N as y}from"./logo-nobg-BVMQOOtC.js";import"./toast-DRvdR0y9.js";import"./logo-yCyWWFG1.js";function v(a={}){const t={...{title:"Transaction Receipt",receiptId:a.id||"",date:a.date||new Date().toLocaleDateString(),time:a.time||new Date().toLocaleTimeString(),amount:a.amount||"0.00",currency:"$",description:a.description||"",senderName:a.by||"",recipientName:a.to||"",bankName:a.bankName||"",accountNumber:a.accountNumber||"",transactionType:a.type||"",status:a.status||"Completed",referenceNumber:a.id||"",fees:a.fees||"0.00",totalAmount:a.amount||"",companyName:"Zenus Bank",companyAddress:"123 Main St, City, Country",companyPhone:"+1 (555) 123-4567",companyEmail:"zenusbanking@gmail.com",additionalFields:a.additionalFields||{},showFooter:!0,footerText:"Thank you for banking with us!"},...a};return`
    ...
    `}const T=async()=>{u("Account Summary");const a=f(),c=await r.auth.getSession();if(!c.data.session){window.location.href="/login";return}const{user:t}=c.data.session,{data:o}=await r.from("profiles").select("*").eq("id",t.id).single(),{data:n}=await r.from("accounts").select("*").eq("user_id",t.id).single(),{data:i}=await r.from("transactions").select("*").eq("account_id",n?.id).order("created_at",{ascending:!1}),d=e=>typeof e=="number"?e.toLocaleString("en-US",{style:"currency",currency:"USD",minimumFractionDigits:2}):e||"$0.00",p=i&&i.length?i.map((e,s)=>`
        ...
        `,pageEvents:x}};export{T as default};
