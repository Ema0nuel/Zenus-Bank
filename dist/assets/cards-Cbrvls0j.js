import{s as c}from"./supabaseClient-B1HaFb4P.js";import{n as _}from"./Navbar-DiGZTYQT.js";import{s as g}from"./toast-DRvdR0y9.js";import{r as $}from"./reset-CYKpHJhn.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm";import"./logo-yCyWWFG1.js";function E(){let t="";for(let a=0;a<16;a++)t+=Math.floor(Math.random()*10);return t.replace(/(.{4})/g,"$1 ").trim()}function S(){return""+Math.floor(100+Math.random()*900)}function L(){const t=new Date,a=t.getFullYear()+4;return`${(t.getMonth()+1).toString().padStart(2,"0")}/${a.toString().slice(-2)}`}function w({card_number:t,card_type:a,card_holder:i,expiry:n,cvv:v}){const o=a==="visa"?'<span class="font-bold italic tracking-tighter text-white text-2xl">VISA</span>':`<div class="flex items-center space-x-1">
  ...
  `}function k(t){if(!t)return"";const a=new Date(t);return`${a.getMonth()+1}/${a.getFullYear().toString().slice(-2)}`}const N=async()=>{$("Zenus Bank | Cards");const t=_(),a=await c.auth.getSession();if(!a.data.session){window.location.href="/login";return}const{user:i}=a.data.session,{data:n}=await c.from("profiles").select("*").eq("id",i.id).single();let{data:v}=await c.from("accounts").select("*").eq("user_id",i.id).single(),{data:o}=await c.from("cards").select("*").eq("user_id",i.id).order("issued_at",{ascending:!1});const f=(o||[]).length?o.map((d,m)=>`
    ...
    `,pageEvents:C}};export{N as default};
