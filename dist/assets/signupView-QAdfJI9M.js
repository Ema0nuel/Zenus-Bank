import{n as T}from"./navbar-CNAggbqQ.js";import{s as l}from"./toast-DRvdR0y9.js";import{a as P,e as f}from"./spinner-KTJn75tI.js";import{r as L}from"./reset-CYKpHJhn.js";import{s}from"./supabaseClient-B1HaFb4P.js";import{s as z}from"./email-cNRN05tX.js";import{N as R}from"./logo-nobg-BVMQOOtC.js";import{L as K}from"./logo-yCyWWFG1.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm";import"./sendEmail-89Z52C2k.js";async function U(t){try{const o=await(await fetch(`https://ipapi.co/${t}/json/`)).json();return{city:o.city,region:o.region,country:o.country_name}}catch{return{city:"",region:"",country:""}}}async function q(t,a){const{email:o,password:n,firstname:e,lastname:i,title:d,phone:p,country_code:r,nationality:u,address:c,city:x,state:S,zip:w,dob:M,occupation:A,ssn:B,marital_status:C,gender:N,username:D,acctype:v}=t,{data:I,error:b}=await s.auth.signUp({email:o,password:n});if(b)throw new Error(b.message);const y=I.user.id,G={id:y,full_name:`${e} ${i}`,title:d||null,firstname:e,lastname:i,phone:p||null,country_code:r||null,nationality:u||null,address:c||null,city:x||null,state:S||null,zip:w||null,dob:M||null,occupation:A||null,ssn:B||null,marital_status:C||null,gender:N||null,email:o,username:`${e} ${i}`||null},{error:m}=await s.from("profiles").insert(G);if(m)throw new Error(m.message);const g="10"+Math.floor(1e9+Math.random()*9e9),{error:h}=await s.from("accounts").insert({user_id:y,account_type:v,account_number:g,balance:0,interest_rate:v==="USD SAVING"?2.5:4});if(h)throw new Error(h.message);const E=await U(a);await z({name:`${e} ${i}`,email:o,accountNumber:g,ip:a,geo:E}),window.location.href="/user/auth"}function k(t){let a=0;if(!t)return{score:a,label:"Empty",color:"bg-gray-300"};if(t.length>=8&&a++,/[A-Z]/.test(t)&&a++,/[a-z]/.test(t)&&a++,/[0-9]/.test(t)&&a++,/[^A-Za-z0-9]/.test(t)&&a++,a<=2)return{score:a,label:"Weak",color:"bg-red-500"};if(a===3)return{score:a,label:"Medium",color:"bg-yellow-500"};if(a>=4)return{score:a,label:"Strong",color:"bg-green-500"}}function F(t){const a=[{id:"firstname",name:"First Name"},{id:"lastname",name:"Last Name"},{id:"email",name:"Email"},{id:"password",name:"Password"},{id:"title",name:"Title"},{id:"selectcountry",name:"Country Code"},{id:"phone1",name:"Telephone"},{id:"country",name:"Nationality"},{id:"uaddress",name:"Address"},{id:"city",name:"City"},{id:"state",name:"State"},{id:"zip",name:"Zip Code"},{id:"dob",name:"Date of Birth"},{id:"occupation",name:"Occupation"},{id:"ssn",name:"SSN"},{id:"marital",name:"Marital Status",type:"radio"},{id:"gender",name:"Gender",type:"radio"},{id:"acctype",name:"Account Type",type:"radio"},{id:"question",name:"Security Question"},{id:"answer",name:"Security Answer"},{id:"account_pin",name:"Account Pin"}];for(const o of a){let n;if(o.type==="radio"?n=t.querySelector(`input[name="${o.id}"]:checked`):n=t.querySelector(`#${o.id}`),!n||!n.value)return l(`${o.name} is required.`,"error"),n?.focus(),!1;if(o.id==="email"&&!/^[\w-.]+@([\w-]+\.)+[\w-]{2,}$/.test(n.value))return l("Invalid email address.","error"),n.focus(),!1;if(o.id==="phone1"&&!/^[0-9 ]{5,15}$/.test(n.value))return l("Invalid phone number.","error"),n.focus(),!1;if(o.id==="zip"&&!/^[0-9]{5,8}$/.test(n.value))return l("Invalid zip code.","error"),n.focus(),!1;if(o.id==="password"&&k(n.value).score<4)return l("Password is too weak. Use upper, lower, number, symbol.","error"),n.focus(),!1}return l("All fields look good!","success"),!0}const $=()=>{L("Signup for Zenus Bank");const t=T();async function a(){t.pageEvents?.();const o=document.getElementById("password"),n=document.getElementById("showHide");n&&o&&n.addEventListener("click",function(){o.type==="password"?(o.type="text",n.textContent="HIDE"):(o.type="password",n.textContent="SHOW")});const e=document.getElementById("passwordStrengthBar"),i=document.getElementById("passwordInfo");o&&e&&i&&o.addEventListener("input",()=>{const{score:p,label:r,color:u}=k(o.value);e.style.width=`${p*25}%`,e.className=`h-2 rounded transition-all duration-500 ${u}`,i.textContent=r==="Empty"?"":r,i.className="text-xs font-bold transition-colors duration-300 ml-1"});const d=document.getElementById("register");d&&d.addEventListener("submit",async p=>{if(P(),p.preventDefault(),!F(d)){f();return}try{const r=Object.fromEntries(new FormData(d)),u=await fetch("https://api.ipify.org?format=json").then(c=>c.json()).then(c=>c.ip);await q(r,u),l("Signup successful! Please check your email for verification.","success")}catch(r){l(r.message||"Signup failed.","error")}finally{f()}})}return{html:`
          ...
          <div class="w-full max-w-5xl space-y-8">
            <div class="flex flex-col items-center mb-6">
              <a href="/" data-nav class="flex items-center justify-center mb-4">
                <img src="${R}" alt="Zenus Bank logo" class="h-16 w-auto block dark:hidden" />
    ...
    `,pageEvents:a}};export{$ as default};
