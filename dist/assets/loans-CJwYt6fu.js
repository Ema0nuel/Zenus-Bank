...
`),g=new Blob([b],{type:"text/csv"}),m=URL.createObjectURL(g),p=document.createElement("a");p.href=m,p.download="loans.csv",p.click(),URL.revokeObjectURL(m)}const J=async()=>{if(!await T())return{html:"",pageEvents:()=>{}};let{data:a=[]}=await u.from("loan").select("*").order("created_at",{ascending:!1}).limit(100),{data:s=[]}=await u.from("profiles").select("id,full_name,email"),{data:n=[]}=await u.from("accounts").select("*"),l=a,b="loans",g=!1,m=localStorage.getItem("admin_dark")==="true";function p(){document.getElementById("app").innerHTML=`
