import{n as g}from"./navbar-CNAggbqQ.js";import{r as p}from"./reset-CYKpHJhn.js";import{s as i}from"./toast-DRvdR0y9.js";import{a as x,e as c}from"./spinner-KTJn75tI.js";import{s as v}from"./supabaseClient-CL6H8VOx.js";import{N as h}from"./logo-nobg-BVMQOOtC.js";import{L as w}from"./logo-yCyWWFG1.js";const M=()=>{p("Forgot Password");const m=g();function u(){m.pageEvents?.();const o=document.getElementById("forgot-form"),r=document.getElementById("email"),e=o?.querySelector('button[type="submit"]'),b=60;let l=null;function d(a){if(!e)return;let t=a;e.disabled=!0;const s=e.getAttribute("data-original")||e.innerHTML;e.setAttribute("data-original",s),e.innerHTML=`Please wait ${t}s`,clearInterval(l),l=setInterval(()=>{t-=1,t<=0?(clearInterval(l),e.disabled=!1,e.innerHTML=s):e.innerHTML=`Please wait ${t}s`},1e3)}r&&r.addEventListener("input",a=>{const t=/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a.target.value);r.style.borderColor=t?"":"rgb(239, 68, 68)",e&&(e.disabled=!t)}),o&&o.addEventListener("submit",async a=>{if(a.preventDefault(),!e||e.disabled)return;const t=r.value.trim();if(!(!t||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t))){d(b),e.innerHTML='<i class="fas fa-spinner fa-spin"></i> Sending...',x();try{const s=(typeof window<"u"?window.location.origin:"https://zenusbanking.com")+"/reset-password",{error:n}=await v.auth.resetPasswordForEmail(t,{redirectTo:s});if(c(),n){n?.status===429||/too many requests/i.test(n.message||"")?i("Too many requests. Please wait a minute and try again.","error"):(i(n.message||"Failed to send reset instructions","error"),d(10));const f=e.getAttribute("data-original")||'Send Instructions <i class="fas fa-paper-plane text-sm"></i>';e.innerHTML=e.disabled?e.innerHTML:f}else i("Password reset instructions sent to your email","success"),setTimeout(()=>window.location.href="/login",2e3)}catch{c(),i("Unexpected error. Try again later.","error"),d(10)}}})}return{html:`
    ...
    `,pageEvents:u}};export{M as default};
