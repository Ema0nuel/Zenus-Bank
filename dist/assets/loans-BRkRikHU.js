import{s as c}from"./supabaseClient-B1HaFb4P.js";import{A as D}from"./AdminNavbar-DXVtneOk.js";import{r as B}from"./adminAuth-Dn35BI8v.js";import{s as f}from"./toast-DRvdR0y9.js";import{s as _}from"./sendEmail-89Z52C2k.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm";import"./logo-nobg-BVMQOOtC.js";import"./logo-yCyWWFG1.js";function y(a){if(!a)return"";const s=new Date(a);return s.toLocaleDateString(void 0,{year:"numeric",month:"short",day:"numeric"})+" "+s.toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit"})}function x(a){return a==="pending"?'<span class="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">Pending</span>':a==="approved"?'<span class="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">Approved</span>':a==="disbursed"?'<span class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">Disbursed</span>':a==="rejected"?'<span class="bg-red-100 text-red-800 px-2 py-1 rounded text-xs">Rejected</span>':a==="closed"?'<span class="bg-gray-100 text-gray-800 px-2 py-1 rounded text-xs">Closed</span>':`<span class="bg-gray-100 text-gray-800 px-2 py-1 rounded text-xs">${a}</span>`}function R(a,s){return`
    ...
    <div>
      <div class="block md:hidden">
        ${a.map(t=>{const r=s.find(b=>b.id===t.user_id);return`
  ...
  `}function U(a,s){const t=["Date","User","Amount","Interest","Status"],r=a.map(v=>{const h=s.find($=>$.id===v.user_id);return[y(v.created_at),h?.full_name||"",v.amount,v.interest_rate||"",v.status].join(",")}),b=[t.join(","),...r].join(`
`),g=new Blob([b],{type:"text/csv"}),m=URL.createObjectURL(g),p=document.createElement("a");p.href=m,p.download="loans.csv",p.click(),URL.revokeObjectURL(m)}const Y=async()=>{if(!await B())return{html:"",pageEvents:()=>{}};let{data:a=[]}=await c.from("loan").select("*").order("created_at",{ascending:!1}).limit(100),{data:s=[]}=await c.from("profiles").select("id,full_name,email"),{data:t=[]}=await c.from("accounts").select("*"),r=a,b="loans",g=!1,m=localStorage.getItem("admin_dark")==="true";function p(){document.getElementById("app").innerHTML=`
      ${D({activeItem:b,isCollapsed:g,isDark:m})}
    ...
    `,document.getElementById("admin-sidebar");const v=document.getElementById("admin-sidebar-overlay"),h=document.getElementById("admin-sidebar-toggle"),$=document.getElementById("admin-sidebar-close");function S(){g=!1,p()}function L(){g=!0,p()}h?.addEventListener("click",S),$?.addEventListener("click",L),v?.addEventListener("click",L),document.getElementById("admin-theme-toggle")?.addEventListener("click",()=>{m=!m,localStorage.setItem("admin_dark",m?"true":"false"),m?document.documentElement.classList.add("dark"):document.documentElement.classList.remove("dark"),p()}),document.querySelectorAll("[data-nav]").forEach(d=>{d.addEventListener("click",i=>{i.preventDefault(),b=d.getAttribute("data-nav"),window.location.href=`/admin/${b}`})}),document.getElementById("admin-logout")?.addEventListener("click",()=>{sessionStorage.removeItem("admin_logged_in"),window.location.href="/admin-login"}),m?document.documentElement.classList.add("dark"):document.documentElement.classList.remove("dark");const j=document.getElementById("loan-search"),E=document.getElementById("loan-status-filter"),k=document.getElementById("loan-table-body");function A(){let d=j.value.trim().toLowerCase(),i=E.value;r=a.filter(e=>{const n=s.find(l=>l.id===e.user_id);let o=!0;return d&&(o=n?.full_name?.toLowerCase().includes(d)||n?.email?.toLowerCase().includes(d)||(e.amount+"").includes(d)||(e.status||"").toLowerCase().includes(d)),i&&e.status!==i&&(o=!1),o}),window.innerWidth<768?document.querySelector(".block.md\\:hidden").innerHTML=r.map(e=>{const n=s.find(o=>o.id===e.user_id);return`
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow p-4 mb-4 animate-fade-in">
              <div class="flex justify-between items-center mb-2">
          ...
          `}).join(""):k&&(k.innerHTML=r.map(e=>{const n=s.find(o=>o.id===e.user_id);return`
            <tr>
          ...
          `}).join("")),I()}[j,E].forEach(d=>{d&&d.addEventListener("input",A),d&&d.addEventListener("change",A)}),document.getElementById("loan-export-csv").onclick=()=>U(r,s);function I(){document.querySelectorAll(".loan-view").forEach(d=>{d.onclick=()=>{const i=d.getAttribute("data-id"),e=a.find(o=>o.id===i),n=s.find(o=>o.id===e.user_id);document.getElementById("loan-detail-panel").innerHTML=q(e,n),document.getElementById("close-loan-detail").onclick=()=>{document.getElementById("loan-detail-panel").innerHTML=""}}}),document.querySelectorAll(".loan-approve").forEach(d=>{d.onclick=async()=>{const i=d.getAttribute("data-id"),e=prompt("Enter interest rate (%)","5");if(!e||isNaN(e))return f("Interest rate required","error");try{const n=a.find(w=>w.id===i),o=s.find(w=>w.id===n.user_id),l=t.find(w=>w.user_id===n.user_id);if(!l)return f("No account found for user","error");const u=parseFloat(l.balance)+parseFloat(n.amount);await Promise.all([c.from("loan").update({status:"approved",interest_rate:parseFloat(e),disbursed_at:new Date().toISOString()}).eq("id",i),c.from("accounts").update({balance:u}).eq("id",l.id),c.from("transactions").insert([{user_id:n.user_id,account_id:l.id,type:"loan_disbursement",amount:n.amount,description:`Loan approved and disbursed with ${e}% interest rate`,balance_before:l.balance,balance_after:u,status:"completed"}])]),await _({to:o.email,subject:"Loan Approved and Disbursed",html:`
          ...
          <ul>
            ...
            <li>Interest Rate: <b>${e}%</b></li>
            <li>New Account Balance: <b>$${parseFloat(u).toLocaleString()}</b></li>
          </ul>
          <p>The funds have been added to your account.</p>
        `}),f("Loan approved, disbursed and user notified.","success"),window.location.reload()}catch(n){console.error(n),f("Error processing loan approval: "+n.message,"error")}}}),document.querySelectorAll(".loan-disburse").forEach(d=>{d.onclick=async()=>{const i=d.getAttribute("data-id"),e=a.find(u=>u.id===i),n=t.find(u=>u.user_id===e.user_id);if(!n)return f("No account found for user","error");const o=parseFloat(n.balance)+parseFloat(e.amount);await c.from("accounts").update({balance:o}).eq("id",n.id),await c.from("loan").update({status:"disbursed"}).eq("id",i),await c.from("transactions").insert([{user_id:e.user_id,account_id:n.id,type:"loan_disbursement",amount:e.amount,description:"Loan disbursed",balance_before:n.balance,balance_after:o,status:"completed"}]);const l=s.find(u=>u.id===e.user_id);await _({to:l.email,subject:"Loan Disbursed",html:`<p>Dear ${l.full_name},<br>Your loan of <b>$${e.amount}</b> has been disbursed to your account.</p>`}),f("Loan disbursed, transaction logged, user notified.","success"),window.location.reload()}}),document.querySelectorAll(".loan-reject").forEach(d=>{d.onclick=()=>{const i=d.getAttribute("data-id");document.getElementById("reject-panel").innerHTML=F(i),document.getElementById("close-reject-modal").onclick=()=>{document.getElementById("reject-panel").innerHTML=""},document.getElementById("reject-form").onsubmit=async function(e){e.preventDefault();const n=this.reason.value.trim();if(!n)return f("Reason required","error");await c.from("loan").update({status:"rejected",reason:n}).eq("id",i);const o=a.find(u=>u.id===i),l=s.find(u=>u.id===o.user_id);await _({to:l.email,subject:"Loan Request Rejected",html:`<p>Dear ${l.full_name},<br>Your loan request was rejected. Reason: <b>${n}</b></p>`}),f("Loan rejected and user notified.","success"),window.location.reload()}}})}I()}return{html:"",pageEvents:()=>p()}};export{Y as default};
