import{s as r}from"./supabaseClient-B1HaFb4P.js";import{s as b}from"./toast-Dx2DSKhR.js";import{s as h}from"./sendEmail-89Z52C2k.js";import{r as w}from"./reset-CYKpHJhn.js";import{n as k}from"./Navbar-DzFN-yR7.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm";import"./logo-yCyWWFG1.js";const E="zenusbanking@gmail.com",M=async()=>{const f=k();w("Zenus Bank | Contact Us");const m=await r.auth.getSession();if(!m.data.session){window.location.href="/login";return}const{user:u}=m.data.session,{data:g}=await r.from("profiles").select("*").eq("id",u.id).single();let d=[];async function x(){const{data:s}=await r.from("notifications").select("*").eq("user_id",u.id).order("created_at",{ascending:!1}).limit(20);d=s||[]}function p(){const s=document.getElementById("notif-view");if(!s)return;s.innerHTML=`
